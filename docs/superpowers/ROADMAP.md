...
| Item | Source | Status |
|---|---|---|
| CERTIFICATION WALL (constellation by real statistics; acquisition-as-choreography; two-voice marks; absent-sidecar state first-class) | D4 | PLANNED v0.8 — design-of-record `.superpowers/sdd/consult-d4-certification-wall.md`; EXISTENCE CONDITION **MET 2026-07-09** (1367433: 5-seed robust starter 42–46 + pins excerpt + gating note, v9-certified, dirty=false); builds against real bytes after the v0.7 T1 v9 re-sync; the 50-seed aggregate (seeds 47–91) = standing follow-up ask (the D2 verdict-grade endgame, not existence) |
| Campaign sibling pipeline: persistent worker + RunSummary + drill-in through untouched hero path; NEVER useRun×N | S2-cliff | PLANNED |
