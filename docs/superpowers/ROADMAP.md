...
| W5 | THE COMMS LENS + THE HERO: the one lost packet (f4 tk30), latency lane, ledger-by-scrub; drop wears the quality register never ✗; in-shader visibility (the named frame-path risk) | SHIPPED + LIVE (merged d3792fc; snapshot #6 687aeea — the repo's deepest review: 15 rounds, ~40 real findings; the pulses live in the shader [precompiled attributes + one playhead uniform]; the hero cadence-safe [collapse + declared afterglow]; every claim completeness-gated; the anchor labeled t30 · LOSS via the post-merge rider [design-bench finding; sub-bloom by arithmetic]) |
| W6 | BELIEF vs REALITY (honest): covEllipse.ts leaf, the f3a shrinking disc; sent-vs-arrived degeneration on f4; no fabricated ✓ | SHIPPED + LIVE (merged 4980b68; snapshot #6 — 2 rounds + ship-bar; the disclosure upgrade UNCOVERED the story: reported 1σ tightens 1.83→0.44m while actual error grows 0.23→2.43m — the truth ends ~5.5σ outside, the tracker overconfident, the track times out; the ν cross-pin honestly REFUSED [innovation ≠ estimate error]; numerics cancellation-free) |
| W7 | THE RAW EVIDENCE TABLE: the byte-X-ray interrogation surface, all six runs | SHIPPED + LIVE (merged fd27428; snapshot #6 — 2 rounds + ship-bar; a modal peer to Hangar/Wall; scope-labeled whole-run/revealed modes with the no-spoiler chip pin; keyboard-reachable full precision; ONE select path) |
| W8 | THE DRONE SILHOUETTE (owner ask, G-2 approved): oriented flat delta, yaw-only from the already-decoded heading; migration table per the design consult | SHIPPED (merged 045f4f1; 1 round ZERO findings — and the wave FOUND A REAL BUG: the shipped heading rotation was MIRRORED (−headingRad reflects the velocity), invisible for the app's entire life because a cone has no nose; corrected + pinned with the discriminating witness; the review independently derived the sign from the contract) |
