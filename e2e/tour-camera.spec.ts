...
// evidence — the AFTER frames; the per-beat screenshot set), and pin the headline behaviours the
...
//
// HARDENING: every sampled beat now (a) GATES on the beat's destination TICK before it reads
...
// The frame-liveness gate is what makes this NOT accept a stalled render — a freeze repeats the same camera
// position, which the earlier settle() latched as "settled"; requiring cur.f > prev.f rejects that. A generous
// deadline THROWS on a real freeze or a camera that never converges (the earlier code silently returned the stale
...
// Gate on the beat's DESTINATION tick before sampling: a play beat's flight takes ~WITNESS_SECONDS, so the
...
test('f1 hero tour: authored arrives compose around the head; the front door ENDS on the finale close-up, not the void', async ({ page }) => {
  ...
  await page.screenshot({ path: 'e2e/screenshots/task-v07-4-f1-beat0.png' })
  ...
  await waitForTick(page, 32) // the beat's play destination — gate before sampling so the head is the ARRIVED pose
  const camB1 = await settle(page)
  const headB1 = (await page.evaluate(HEAD_POS)) as [number, number, number]
  await page.screenshot({ path: 'e2e/screenshots/task-v07-4-f1-beat1.png' })
  // The medium compose sits the camera ~HEAD_MEDIUM_DISTANCE (50) off the head (finaleFraming → |cam−head| ≈ 50).
  // TWO-SIDED: the lower bound rejects the legacy auto-follow ending ~18u off the head (the old one-sided < 90
  ...
  // MINIMUM DISPLACEMENT: the arrive actually MOVED the camera off the beat-0 hero frame (a stale/un-applied
  ...
  await page.screenshot({ path: 'e2e/screenshots/task-v07-4-f1-beat2.png' })
  // The finale close-up sits ~FINALE_DISTANCE (25) off the head. TWO-SIDED: the lower bound rejects a
  // collapsed camera; the upper (~34) rejects the ~340u prefix void the earlier front door ended on.
  const dB2 = dist(camB2, headB2)
  // TIGHTENED terminal window: exclude the 18-23u legacy auto-follow close (the authored finale close-up is
  // FINALE_DISTANCE=25). The earlier lower bound of 16 admitted that stale follow ending; 24 rejects it while the
  ...
  // MINIMUM DISPLACEMENT: the terminal arrive moved the camera off beat 1 (measured ~164u).
...
test('f2a sensing tour: opens and closes on the stage bookend, with authored conjunction/head arrives between', async ({ page }) => {
  ...
  await page.waitForFunction('!!window.__scene', undefined, { timeout: 15000 })
  // NEGATIVE CONTROL — the PRE-TOUR load-vantage camera. Beat 0 is UN-AUTHORED (no arrive — the FOCUS PAN is
  ...
  await page.screenshot({ path: 'e2e/screenshots/task-v07-4-f2a-beat0.png' })
  ...
  const camB1 = await settle(page)
  const headB1 = (await page.evaluate(HEAD_POS)) as [number, number, number]
  await page.screenshot({ path: 'e2e/screenshots/task-v07-4-f2a-beat1.png' })
  ...
  // head. TWO-SIDED: the lower rejects the legacy follow-close ending; the upper rejects the ~405u stage / void.
  const dB1 = dist(camB1, headB1)
  ...
  const camB2 = await settle(page)
  const headB2 = (await page.evaluate(HEAD_POS)) as [number, number, number]
  await page.screenshot({ path: 'e2e/screenshots/task-v07-4-f2a-beat2.png' })
  const dB2 = dist(camB2, headB2)
  ...
  // MINIMUM DISPLACEMENT: b2's arrive MOVED the camera off beat 1. This is the headline stale-frame guard —
  ...
  await page.screenshot({ path: 'e2e/screenshots/task-v07-4-f2a-beat3.png' })
  ...
  // MINIMUM DISPLACEMENT: b3's eclipse arrive MOVED the camera off beat 2 (beat 2's camera ~74.5u from beat 3's
  ...
  await page.screenshot({ path: 'e2e/screenshots/task-v07-4-f2a-beat4.png' })
  ...
  // MINIMUM DISPLACEMENT: b4's head-medium arrive MOVED the camera off beat 3 (measured ~43u).
  ...
  await page.screenshot({ path: 'e2e/screenshots/task-v07-4-f2a-beat5.png' })
  expect(dist(camB5, stageVantage), `f2a b5 returns to the stage bookend (Δ=${dist(camB5, stageVantage).toFixed(1)}u)`).toBeLessThan(5)
  // MINIMUM DISPLACEMENT: the crane-back to the stage MOVED the camera off the beat-4 head-medium (measured ~272u).
...
// SwiftShader. The three AFTER frames are the browser evidence (the three e0 shot captures).
test('e0 query-stage tour: corridor / crane / stage-bookend arrives each move the camera; beat 5 returns to the load bookend', async ({ page }) => {
  ...
  const camB2 = await settle(page)
  await page.screenshot({ path: 'e2e/screenshots/task-v08-w7-e0-shot1.png' })
  ...
  const camB4 = await settle(page)
  await page.screenshot({ path: 'e2e/screenshots/task-v08-w7-e0-shot2.png' })
  ...
  const camB5 = await settle(page)
  await page.screenshot({ path: 'e2e/screenshots/task-v08-w7-e0-shot3.png' })
