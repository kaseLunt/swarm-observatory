...
export const TOURS: Record<string, Tour> = {
  e0: {
    ...
    steps: [
      ...
      { play: { to: 43, speed: 4 }, caption: 'Act II — sightlines from the origin. A line of sight reaches for a far target and dies at the first solid in its way: at tick 39 the sphere blocks it, and the sightline stops dead at the contact where it hit.', holdMs: 10400 },
      ...
      { play: { to: 74, speed: 8 }, caption: 'Act III — a second observer, drawn per-seed at n=−601, interrogates the same world from a new vantage. After the tour, press O to stand at its eye and look down the sightline.', holdMs: 8800 },
      { tick: 74, select: { event: 74 }, caption: 'The closing beat — a clear line of sight sails clean through empty space. One run, 75 queries, self-checked on load. Explore freely: every view is a shareable URL.', holdMs: 8400 },
    ],
  },
  f1: {
    id: 'f1-motion',
  ...
  f2a: {
    ...
    steps: [
      { tick: 0, select: { entity: '1:0', event: null }, caption: 'A single drone in real recorded flight, watched by a fixed sensor at the origin. Its field-of-view cone, range ring and the occluder sphere are scenario constants; the flight itself is decoded-real.', holdMs: 9800 },
      { tick: 48, select: { event: 99 }, arrive: { kind: 'conjunction' }, caption: 'In range, and line of sight is clear — but the drone is still OUTSIDE the field-of-view cone, so the sensor does not admit it. In range and LOS clear are recomputed live; in FOV is the claim voice — a pinned angle, no bearing in the bundle to recompute.', holdMs: 12600 },
