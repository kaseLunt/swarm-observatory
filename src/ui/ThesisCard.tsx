...
// COLLAPSED FORM (v0.7 T5, critic R6): once the auto-tour leaves its opening establishing beat the full card
...
// chrome (#20). The verdict WITHHOLDS exactly as the card does (null → no glyph/headline); × always renders.
...
// THE ZERO-CLICK THESIS CARD (v0.6 T6, P2 — the cold-open share surface). One card that states the thesis
...
// the share affordance until dismissed with ×. DOM only — no WebGL, no frame path (§8).
//
// verification state NEVER rides the URL (the NEVER-list): the copy handler builds the link from the VIEW
...
export function ThesisCard({ verdict, onCopyLink, onDismiss }: {
  // TrustVerdict = the run's real trust verdict (three-voice grammar); null = WITHHELD (W1 fail-safe): the
  ...
  return (
    <section className="thesis-card" aria-label="run verdict and share">
      ...
      {/* Verdict + subline render ONLY when a verdict is known (v non-null). When WITHHELD they are omitted
          entirely — the honest blank of the W1 fail-safe. The independence line (a static claim about how
