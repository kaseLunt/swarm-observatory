...
// The composed resting frame: camera high on the +x/+y/+z octant looking at a point lifted
...
// parameter is StateFrame, not a raw number: RunModel.entityStatesAt is branded, and typing the structural
...
// ── The DIRECTED-CAMERA ANCHOR (finale close-up / authored-shot head / follow aim) ─────────────────────
...
// absent) keeps the centroid, byte-identical to the prior centroid path. Scene calls this at its two EVENT-RATE sites (finale,
// authored-shot head — where the prior code already allocated the same `[cx/count, …]` tuple); the per-frame
// follow reads the same `useSubject` boolean into zero-alloc number locals (the load budget), so no new frame-path allocation.
...
// ── The SUBJECT-POSE HOLD, TIMELINE-DERIVED (a temporary dropout must not yank the directed camera off the
...
// trail. Mutates `out` in place and returns has — zero allocation, frame-path safe (the load budget). `TrailView` is the
...
// `frame` is a StateFrame (uniform with entityStatesAt): the anchor is looked up at the evaluated frame t0
...
// Derive the same bounding sphere DIRECTLY from a prebuilt trail's interleaved xyz Float32Array.
// buildTrail already walks every tick once at model publish to lay down its vertices, so the
...
//
// PREFIX COUNT: `count` need not be the full vertex count. The trail-frame arrival ease (Scene)
// passes count = arrivedTick + 1 to fit the trajectory-SO-FAR — the box of vertices 0..count-1,
...
// Aspect-aware fit distance (v0.7 fixwave). fitDistance above uses the VERTICAL fov only — conservative
...
// `elevationDeg` is an OPTIONAL stage-local vantage elevation for the OVERSIZED-fit regime (the whole-path
...
// `aspect` (v0.7 fixwave) is an OPTIONAL live width/height: when present it tightens the fit against the
...
export function frameFor(bounds: Bounds | null, opts: FrameOpts, aspect?: number): Framing {
  ...
  // distance from the subject. The prior rider re-aimed the pivot at bounds.center but LEFT the camera
  ...
  // Unit view direction from the default camera (origin → default position). opts.elevationDeg, when set,
...
// Compose-around-head FINALE framing (a design ruling). At a NATURAL play-to-end rest the resting subject
...
// ── Authored tour-camera shot resolution (v0.7) ───────────────────────────────────────────────────────
...
//   VISUAL EXTENTS (v0.7 fixwave): `headRadius`/`sensorRadius` are the on-screen radii of the drone
...
export interface ShotAnchors {
  ...
  // e0 query-stage authored shots (v0.8). Both are DECODE-TRUE, composed in queryScene from the parsed draws
...
export interface ShotOpts {
  ...
  // OPTIONAL stage-fit opts for the 'stage' shot ONLY. The 'stage' bookend must land byte-identically on the
  ...
  // ⇒ the 'stage' case falls back to `fit`, byte-identical to the earlier behavior (and to every non-sensing 'stage').
  stage?: FrameOpts
}

// Append a point's axis-aligned ±radius extent (its six octahedral extremes) to `out`, so a marker's VISUAL
// size — not just its centre — enters the conjunction box→sphere fit. radius 0 collapses to six copies of
...
// Resolve an authored TourShot to a Framing from LIVE scene anchors (v0.7). PURE — the descriptor carries
...
//   `aspect` (v0.7 fixwave) is the live canvas width/height, threaded ONLY into the CONJUNCTION fit so a
...
export function shotFraming(shot: TourShot, a: ShotAnchors, o: ShotOpts, aspect?: number): Framing | null {
  switch (shot.kind) {
    ...
    case 'conjunction': {
      ...
      // extent when the eclipse variant asks], each bounded by its VISUAL extent (the marker radii), then
...
// Revealed-trail midpoint VERTEX index for the follow-aim bias (a design ruling). The playback follow aimed
// dead at the live head, so the one-sided trail — which lies entirely BEHIND the head — projected off to
// one side of frame. The fix biases the pivot off the head toward the midpoint of the path
...
// Absolute world-space CAP on the follow-aim bias displacement (a design-ruling fixwave). Companion to
...
// Predictive follow-lead displacement (a design ruling). The exponential playback follow (followPan at the
...
// toward the leading frame edge (an observed note: head at ~89-93% frame width, the label clipping). Cancel most
...
export function followLead(
  ...
  // FINITE GUARD (finaleFraming precedent). The exported pure surface must never propagate
...
// Unselected mid-run tracking-ring gate (design rulings). During unselected play
...
//
//   A design ruling — PAUSE PERSISTENCE. pause-then-click is the sanctioned mid-run selection path, but the
...
// Distance-proportional tracking-ring SCALE (a design ruling). The v0.5c tracking marker was a FIXED world
...
// Aspect-compensated effective follow-lead (a design ruling; re-measured — Scene.tsx CALIB_ASPECT is the
...
// the reserved 280px|minmax|320px grid makes that canvas 840×687, so the canvas aspect ≈1.223 is well below
...
// clipping edge (the six-window re-sweep: 1280×720 → 0.749 and the full-width-drawer layout → 0.711 hold the ≤0.75
...
// pulls back toward centre) — the OPPOSITE of the design ruling's illustrative `baseLead·min(1, aspect/calibAspect)`, which
...
// is bounded in practice: the flanked-narrow layout floors at the ≈0.78 drawer-breakpoint canvas (the reserved-
