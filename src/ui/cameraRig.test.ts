...
// W4 — the shared load / tour-start framing decision. Scene threads ONE activeStageBounds (the query core
// theatre for e0, the sensing scope for f2a, null otherwise) into BOTH CameraRig's load write and Entities'
// tour-start reset via this helper, so step 0 opens byte-identically on the load frame. The pre-W4 bug: the
...
describe('loadFraming (shared load/tour-start framing — W4)', () => {
  const sensing: Bounds = { center: [48, 0, 21], radius: 130 } // f2a-shaped: the sensing scope, hundreds of units across
  const trail: Bounds = { center: [0, 0, 20], radius: 95 }      // the plain trajectory bounds the pre-W4 f2a tour start cut to
  ...
  test('the sensing bounds, NOT the trail bounds, drive the frame when stageBounds is present (the W4 regression)', () => {
    // If the tour-start reset fell back to the trail bounds (the pre-W4 f2a bug) it would frame
  ...
  // G6 — the sensing stage's RAISED resting vantage. The default STAGE fit rests ~22.6° above the deck, where the
